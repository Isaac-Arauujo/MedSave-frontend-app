...
const formatBatchLabel = (batchNumber?: string) => batchNumber?.trim() || 'Lote não informado';
                ...
                {listings.map((listing) => (
                  <tr key={listing.id}>
                    ...
                    <td className="px-4 py-3 text-sm text-on-surface-variant">
                      {formatBatchLabel(listing.batchNumber)}
                    </td>
                    <td className="px-4 py-3 text-sm text-on-surface-variant">
            ...
            {listings.map((listing) => (
              ...
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-medium text-on-surface">{listing.product.name}</h3>
                    <p className="text-sm text-on-surface-variant">
                      Lote: {formatBatchLabel(listing.batchNumber)}
                    </p>
                    <p className="text-sm text-on-surface-variant">
                ...
                <dl className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <dt className="text-on-surface-variant">Preço</dt>
                    <dd className="font-medium text-primary">{formatCurrency(listing.discountPrice)}</dd>
                  </div>
                  <div>
                    <dt className="text-on-surface-variant">Estoque</dt>
                    <dd className="text-on-surface">{listing.availableStock}</dd>
                  </div>
                </dl>
